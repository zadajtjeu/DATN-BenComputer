                ...
                <div class="sidebar">
                    ...
                    <div class="electronics mb-40">
                        ...
                        <div id="shop-cate-toggle" class="category-menu sidebar-menu sidbar-style">
                            <ul>
                                @forelse($category->children as $cate_item)
                                    <li>
                                        <a href="{{ route('categories.details',$cate_item->slug) }}">{{ $cate_item->name }}</a>
                                    </li>
                                @empty
                                @endforelse
                            </ul>
                    ...
                    <div class="search-filter mb-40">
                        ...
                        <form class="sidbar-style" action="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}" method="get">
                    ...
                    <div class="search-filter mb-40">
                        ...
                        <form class="w-100 p-2 me-3 input-group" action="{{ route('categories.details', $category->slug) }}" method="get" style="max-width: 500px;">
                    ...
                    <div class="main-toolbar-sorter clearfix">
                        <div class="toolbar-sorter d-flex align-items-center" >
                            ...
                            <select class="sorter wide" id="sort">
                                ...
                                <option @if ($sort == '') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">{{ __('Popular') }}</option>
                                @php $parameters['sort'] = 'newest' @endphp
                                <option @if ($sort == 'newest') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">{{ __('Newest') }}</option>
                                @php $parameters['sort'] = 'top_seller' @endphp
                                <option @if ($sort == 'top_seller') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">{{ __('Top Seller') }}</option>
                                @php $parameters['sort'] = 'price_asc' @endphp
                                <option @if ($sort == 'price_asc') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">{{ __('Low Price') }}</option>
                                @php $parameters['sort'] = 'price_desc' @endphp
                                <option @if ($sort == 'price_desc') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">{{ __('High Price') }}</option>
                            </select>
                            @php
                                if ($sort == '') {
                    ...
                    <div class="main-toolbar-sorter clearfix">
                        <div class="toolbar-sorter d-flex align-items-center">
                            ...
                            <select class="sorter wide" id="showproduct">
                                ...
                                <option @if ($show == '12') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">12</option>
                                @php $parameters['show'] = '24' @endphp
                                <option @if ($show == '24') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">24</option>
                                @php $parameters['show'] = '36' @endphp
                                <option @if ($show == '36') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">36</option>
                                @php $parameters['show'] = '48' @endphp
                                <option @if ($show == '48') selected @endif value="{{ route('categories.details', array_merge(['slug' => $category->slug], $parameters)) }}">48</option>
                            </select>
                            @php
                                if ($show == '') {
